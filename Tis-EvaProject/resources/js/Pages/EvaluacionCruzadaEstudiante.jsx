...
import "../../css/AutoevaluacionEstudiante.css";
...
const EvaluacionCruzadaEstudiante = () => {
    ...
    useEffect(() => {
        const fetchEvaluacionIndividual = async () => {
            if (!evaluacionCruzada?.ID_EVAL_CRUZADA || !estudianteSeleccionado) return;
            ...
            try {
                ...
                console.error("Error al obtener la evaluación individual:", error);
                setResultado(null);
            }
        };
    ...
    <p><strong>Nota asignada:</strong> {resultado.PUNTUACION_OBTENIDA}</p>
    <p>
        Tu evaluación para este estudiante ha sido registrada exitosamente. 
        Gracias por tu aporte para una evaluación justa y constructiva.
    </p>
</div>
    ...
    <form onSubmit={handleSubmit}>
        {evaluacionCruzada.preguntas.map((pregunta) => (
            <div key={pregunta.ID_PREGUNTA_EVAL} className="autoevaluacion-item">
                <h4 className="pregunta-title">{pregunta.PREGUNTA_EVAL}</h4>
                <div className="opciones-container">
                    {pregunta.opciones.map((opcion) => (
                        <div key={opcion.ID_OPCION_EVAL} className="opcion-col">
                            <span className="opcion-titulo">{opcion.TEXTO_OPCION_EVAL}</span>
                            <label className="opcion-radio">
                                <input
                                    type="radio"
                                    name={`pregunta-${pregunta.ID_PREGUNTA_EVAL}`}
                                    value={opcion.ID_OPCION_EVAL}
                                    onChange={() =>
                                        handleResponseChange(pregunta.ID_PREGUNTA_EVAL, opcion.ID_OPCION_EVAL)
                                    }
                                    checked={respuestas[pregunta.ID_PREGUNTA_EVAL] === opcion.ID_OPCION_EVAL}
                                />
                            </label>
                            <span className="opcion-puntos">{opcion.PUNTUACION_OPCION_EVAL}</span>
                        </div>
                    ))}
                </div>
            </div>
        ...
        <div className="form-footer">
            <button
                ...
                disabled={!estudianteSeleccionado || Object.keys(respuestas).length !== evaluacionCruzada.preguntas.length}
            >
                Guardar Respuestas
            </button>
        </div>
