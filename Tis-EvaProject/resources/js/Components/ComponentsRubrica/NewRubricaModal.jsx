    ...
    const handleSave = async () => {
        ...
        for (let i = 0; i < criterios.length; i++) {
            ...
            for (let j = 0; j < criterio.niveles.length; j++) {
                ...
                if (!nivel.titulo.trim()) {
                    setError(`El nivel ${j + 1} del criterio ${i + 1} debe tener un título.`);
                    return;
                }
    
                if (nivel.puntos === "" || nivel.puntos === null) {
                    setError(`El nivel ${j + 1} del criterio ${i + 1} debe tener un puntaje.`);
                    return;
                }
            }
        }
        ...
        const newRubrica = {
            ...
            criterios: criterios.map((criterio) => ({
                ...
                maxPuntaje: Math.max(...criterio.niveles.map((nivel) => Number(nivel.puntos || 0))),
                niveles: criterio.niveles.map((nivel) => ({
        ...
        try {
            if (isEditMode && rubrica?.ID_RUBRICA) {
                ...
                await axios.post("http://localhost:8000/api/rubricas", newRubrica);
            }
    
            onSave(newRubrica);
            onClose();
        } catch (error) {
            console.error("Error al guardar la rúbrica:", error.response?.data || error);
            setError(
